...
const alert_message = (type, title, quantity, cathegory) => {
  ...
  if (type === 'SUCCESS') {
    if (quantity < 0) {
      isSingular
        ? message.success(title + ' was removed from the ' + cathegory)
        : message.success(abs_quantity + ' ' + title + 's were removed from the ' + cathegory);
    } else if (quantity > 0) {
      isSingular
        ? message.success(title + ' was added to the ' + cathegory)
        : message.success(abs_quantity + ' ' + title + 's were added to the ' + cathegory);
    }
  } else if (type === 'WARNING') {
    message.warning(
      title + " couldn't be added to the " + cathegory + ', because you already have selected the maximu quantity'
    );
  } else {
    isSingular
      ? message.error('An error accrued. ' + title + ' was not added to the ' + cathegory)
      : message.error('An error accrued. ' + quantity + ' ' + title + 's were not added to the ' + cathegory);
  }
};
...
export class AddToCart extends React.PureComponent {
  ...
  render() {
    return (
      <button onClick={this.addToCart} aria-label={'Add to cart'} className="site-btn">
        Add to cart
      </button>
    );
  }
  ...
  render() {
    ...
    return (
      <li>
        ...
        <h6>{title}</h6>
        <p>
          <InputNumber min={1} max={100} defaultValue={currentQuantity} onChange={this.changeQuantity} /> x {currency}
          {subtotalPrice} <RemoveFromCart good_id={good.good._id} quantity={-currentQuantity} title={title} />
        </p>
      </li>
    );
  }
