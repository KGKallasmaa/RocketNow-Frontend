...
export class ParcelDeliveryLocationForm extends React.PureComponent {
  ...
  async handleChange(value) {
    ...
    let variables = {
      jwt_token: isRegularUserLoggedIn()
        ? sessionStorage.getItem('jwtToken')
        : sessionStorage.getItem('temporary_user_id'),
    ...
    if (deliveryEstimateFromDB != null) {
      ...
      this.props.DeliveryEstimateSelected(deliveryEstimate);
    }
  }
  ...
  async getParcellLocations(MyLatitude, MyLongitude) {
    ...
    if (parcelLocationsFromDB !== null) {
      const nearestLocations =
        MyLatitude !== 0 && MyLongitude !== 0 ? parcelLocationsFromDB.ParcelDeliveryLocations.slice(0, 5) : [];
      const otherLocations =
        MyLatitude !== 0 && MyLongitude !== 0
          ? parcelLocationsFromDB.ParcelDeliveryLocations.slice(5, parcelLocationsFromDB.ParcelDeliveryLocations.length)
          : parcelLocationsFromDB.ParcelDeliveryLocations;
  ...
  render() {
    ...
    if (loadingParcelLocations === true) {
      return (
        <div>
          <div className="cf-title">Parcel Delivery</div>
                ...
                <Select
                  ...
                  value={
                    this.props.ParcelDeliveryLocationCountry !== undefined ? (
                      <Emoji
                        text={
                          getEmoji(this.props.ParcelDeliveryLocationCountry) + ' ' + this.props.ParcelDeliveryLocation
                        }
                      />
                    ) : undefined
                  }
                >
                  <OptGroup label="Near you">{NearestLocations}</OptGroup>
                  <OptGroup label="Other locations">{OtherLocations}</OptGroup>
                </Select>
              </div>
            </div>
          </div>
