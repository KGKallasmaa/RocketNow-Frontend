  ...
  if (shippingAddress.shippingMethod === 'ParcelDelivery') {
    return (
      <div>
        ...
        <b>Location:</b> {shippingAddress.parcelDeliveryLocation.name}{' '}
        {getEmoji(shippingAddress.parcelDeliveryLocation.country)}
        <br />
    ...
    return (
      <div>
        <b>To:</b> {shippingAddress.shippingName} <br />
        <b>Address:</b> {shippingAddress.addressOne}
        {addressLine2} <br />
        ...
        {getEmoji(shippingAddress.country)} <br />
      </div>
    );
  }
}
...
function renderOrderItems(order) {
  if (order !== '') {
    return order.order_items.map((item) => {
      return (
        <p>
          {item.quantity} x <b>{item.title} </b> &nbsp;{currency_display_dictionary[item.currency]}{' '}
          {item.price_per_one_item}
        </p>
      );
    });
  }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <Modal title={tile} visible={this.state.visible} onOk={this.handleOk} width={700} onCancel={this.handleCancel}>
          <Steps size="small" current={1}>
            <Step status={receivedStatus} title="Received" subTitle={receivedTime} icon={<Icon type="shopping" />} />
            <Step
              status={processingStatus}
              title="Processing"
              subTitle={order.processing_start_timestamp_UTC !== null ? processingStartTime : ''}
              icon={<Icon type="loading" />}
            />
            <Step
              status={processedStatus}
              title="Proceeded"
              subTitle={order.processing_end_timestamp_UTC !== null ? processingEndTime : ''}
              icon={<Icon type="rocket" />}
            />
            <Step
              status={shippedStatus}
              title="Shipped"
              subTitle={order.shipped_timestamp_UTC !== null ? shippedTime : ''}
              icon={<Icon type="smile" />}
            />
            <Step
              status={shippedStatus}
              title="Estimate"
              subTitle={order.deliveryEstimate_UTC !== null ? deliveryEstimate : ''}
