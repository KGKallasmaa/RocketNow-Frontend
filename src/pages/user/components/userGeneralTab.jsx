  ...
  for (let i = 0; i < addresses.length; i++) {
    ...
    if (singleOrder.isActive === true) {
      ...
      if (singleOrder.shippingMethod === 'AddressDelivery') {
        const string =
          singleOrder.addressOne +
          singleOrder.addressTwo +
          singleOrder.city +
          singleOrder.country +
          singleOrder.shippingName +
          singleOrder.zip;
...
function renderTable(locations) {
  if (locations.length < 1) {
    return (
      <Empty image={noOrders} imageStyle={{ height: 180 }} description={<span>No past delivery locations found</span>}>
    ...
    <Table dataSource={locations}>
        ...
        render={(location) => (
          <span>
            {location.map((location) => (
              <div>{renderShippingAddress(location)}</div>
            ))}
          </span>
        )}
      />
      <Column
        ...
        render={(type) => (
          <span>
            {type.map((type) => (
              <Tag color={type === 'Default' ? 'red' : type === 'AddressDelivery' ? 'blue' : 'green'} key={type}>
                {type}
              </Tag>
            ))}
          </span>
